...
describe('assistant for the open flow', () => {
  it('puts the current flow, the entities and the question into the prompt', () => {
    const prompt = buildAssistInstructions({
      kind: 'automation',
      flowYaml: 'alias: Flur\ntriggers: []\nactions: []',
      request: 'Warum geht das Licht nicht an?',
      entities: [{ entity_id: 'light.flur', name: 'Flur' }],
      language: 'de',
      history: [
    ...
    expect(prompt).toContain('alias: Flur');
    expect(prompt).toContain('- light.flur | Flur');
    expect(prompt).toContain('User: Hallo');
    expect(prompt.trim().endsWith('Warum geht das Licht nicht an?')).toBe(true);
  });

  it('splits a reply into explanation and proposed YAML', () => {
    expect(parseAssistReply('Alles gut.')).toEqual({ text: 'Alles gut.' });
    expect(
      parseAssistReply('- Verzögerung ergänzt\n\n```yaml\nalias: Flur\nactions: []\n```')
    ).toEqual({ text: '- Verzögerung ergänzt', yaml: 'alias: Flur\nactions: []' });
  });
});
