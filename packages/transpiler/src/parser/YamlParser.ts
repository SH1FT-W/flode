...
export class YamlParser {
    ...
    actions.forEach((action, index) => {
      ...
      if (isConditionAction(action)) {
        ...
        try {
          ...
          // Unparseable service call — keep the original step verbatim (lossless).
        ...
        // A step FLODE has no dedicated node for (e.g. the `scene:` shorthand,
        // or something from a newer HA version): keep it verbatim so saving
        // writes it back unchanged — lossless, so no import warning.
        const nodeId = getNextNodeId('action');
        nodes.push(this.createRawStepNode(nodeId, action));
        createEdgesFromCurrent(nodeId);
        currentNodeIds = [nodeId];
  ...
  /**
   * Create a pass-through node that keeps an unmodellable action step verbatim
   * (see `@flode/shared`'s raw-step helpers).
   */
  private createRawStepNode(nodeId: string, step: object): ActionNode {
    return {
      id: nodeId,
      type: 'action',
      position: { x: 0, y: 0 },
      data: createRawStepData({ ...step }),
    };
  }
