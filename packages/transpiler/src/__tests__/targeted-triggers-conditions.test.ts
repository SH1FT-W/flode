...
const isLightTrigger = (obj: Record<string, unknown>) => obj.trigger === 'light.turned_on';
const isVibrationCondition = (obj: Record<string, unknown>) =>
  obj.condition === 'vibration.is_detected';
...
`;
  ...
  it('survives YAML → graph → YAML → graph → YAML unchanged', async () => {
    ...
    for (let pass = 0; pass < 2; pass++) {
      ...
      const triggers = collectObjects(result.output, isLightTrigger);
